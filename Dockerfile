# jayeon WAS(jayeon-api.redhead.kr) — Cloud Run 컨테이너 이미지
#
...
RUN CGO_ENABLED=0 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/jayeon-was .

# --- run: distroless (셸·패키지매니저 없음) ---
FROM gcr.io/distroless/static-debian12:nonroot

# 런타임에 디스크에서 읽는 파일이 없다. 바이너리 하나로 끝난다.
COPY --from=build /out/jayeon-was /app/jayeon-was
