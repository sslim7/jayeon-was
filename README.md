...
│   ├── auth             사용자 로그인·비밀번호 변경·JWT 발급 및 검증, 기기 세션 목록·끊기
│   ├── credentials      사용자와 어드민이 공유하는 비밀번호 해시·이메일 정규화
│   ├── users            Firestore 사용자 저장소(계정 + 기기 세션)와 내 프로필
...
| 경로 | 동작 |
| --- | --- |
| `POST /auth/login` | 이메일·비밀번호 확인 후 토큰과 `mustChangePassword` 반환. **세션(기기) 하나를 만든다** |
| `POST /auth/refresh` | 계정 상태·토큰 버전·**세션 생존** 대조 후 토큰 재발급 |
| `POST /auth/change-password` | 현재 비밀번호 확인 후 변경, 성공 시 204 |
| `GET /auth/sessions` | 로그인된 기기 목록 |
| `DELETE /auth/sessions/{sessionId}` | **그 기기만** 끊기 |
...
이미 발급된 액세스 토큰은 최대 15분 더 유효하다.

### 기기 세션 — 「이 기기만 끊기」

폰을 잃어버렸을 때 예전에는 **비밀번호를 바꿔 모든 기기를 한꺼번에** 로그아웃시키는 길밖에
없었다. 세션은 그 위에 기기 단위를 얹은 것이고, 비밀번호 변경(전체 끊기)은 그대로 있다.

- 로그인 한 번이 `users/{userId}/sessions/{sessionId}` 문서 하나를 만든다. 두 토큰에 그
  세션 id 가 실린다.
- 재발급은 **토큰 버전(계정 전체)과 세션 생존(기기 하나)을 모두** 본다. 하나라도 어긋나면 401.
- 🔴 **세션 id 가 없는 리프레시 토큰은 401 이다.** 세션 장치 이전에 발급된 토큰이며 호환
  경로를 두지 않았다 — 받아 주는 갈래는 곧 세션 검사를 우회하는 길이다.
  **이 기능을 처음 배포한 뒤에는 모든 사용자가 한 번 다시 로그인한다.** 고장이 아니다.
- ⚠️ 기기 이름(`deviceLabel`)은 `User-Agent` 로 **추정**한다. 「Chrome · macOS」,
  「네이처 앱 1.4.0 · 안드로이드」 정도가 한계이고 모델명은 대개 알 수 없다. 모르면
  「알 수 없는 기기」이고, 근거가 된 UA 원문을 함께 돌려준다.
- ⚠️ `lastSeenAt` 은 **날짜 단위로만 정확하다.** 쓰기를 줄이려고 한국 날짜가 바뀌거나 기기
  표시가 달라졌을 때만 기록한다(기기당 하루 1~2회). 그 덕에 최근 기록 10건이 사실상
  「최근 열흘 중 쓴 날」이 된다.

#### 🔴 끊어도 즉시 끊기지 않는다

서버는 액세스 토큰을 저장하지도 검증하지도 않는다(매 요청마다 문서를 읽지 않기 위한 설계).
그래서 세션을 끊어도 **그 기기가 이미 쥔 액세스 토큰은 만료될 때까지 통한다.**
액세스 토큰 수명을 1시간에서 **15분**으로 줄인 것이 그 구멍의 크기를 줄인 것이다.

언제까지인지는 끊기 응답과 목록의 `accessibleUntilAtMost` 에 실린다
(`revokedAt + 15분`). 앱은 이 값으로 「이 기기는 00:49까지 쓸 수 있어요」라고 안내한다.
⚠️ 이름 그대로 **상한**이지 정확한 시각이 아니다 — 그 기기가 마지막으로 재발급한 것이
한참 전이면 실제로는 더 일찍 끊긴다.
...
- 템플릿 CRUD, JPEG/PNG 첨부 업로드·내용 조회·논리삭제. 이미지 개별 700KiB, 최대 3개/합계 1400KiB(서버 저장 한도이며 실제 발송 가능 크기는 단말이 SIM에서 읽는 `MMS_CONFIG_MAX_MESSAGE_SIZE`로 통신사가 정합니다. 그래서 앱이 첨부 시점에 이미지를 줄입니다). 한도·형식 거절은 사유와 실제 값이 담긴 문장으로 돌려줍니다. 아이폰 사진첩 기본 형식인 HEIC를 그대로 올리면 형식을 지목하고 JPEG로 저장하도록 안내합니다. 템플릿·캠페인에는 첨부 메타데이터만 복사하고 실제 이미지는 사용자 하위 별도 Firestore 문서에 보관합니다. Android는 SMS/LMS/MMS를 전송하며 서버는 실제 결과의 `transport`를 저장합니다.
- `.xlsx` 가져오기는 첫 시트 첫 행의 `이름`과 `전화번호` 또는 `연락처` 필수 헤더(영문 name/phone도 가능), 선택 `그룹` 헤더(영문 groupId), 최대 2MiB입니다(행 수는 막지 않고 나누어 저장합니다). 두 번째 행부터 자료를 읽고 이름이나 전화번호 값이 누락된 행은 제외합니다. 그룹은 헤더가 없거나 셀이 비어 있어도 등록할 수 있습니다. `전화번호`와 `연락처`를 동시에 넣으면 별칭 중복 오류입니다. 추가 열은 헤더 순서와 셀 문자열을 customFields로 보존합니다(최대 20개, 행별 JSON 2000bytes). 미리보기는 `recipientImports/{id}/chunks/{n}` 에 200행·500KiB 단위로 나눠 저장하므로 문서 한도(1MiB)에 걸리지 않습니다. 거절은 사유가 담긴 문장으로 돌려줍니다. 전화번호 셀을 텍스트로 작성해 맨 앞 0을 보존하세요. 사전검증 후 24시간 내 확정하고, 기존 번호·파일내 중복·잘못된 행을 제외합니다. 확정은 청크마다 트랜잭션 하나로 처리하며(Firestore 트랜잭션 쓰기 상한 500건), 중간에 끊겨도 같은 import ID로 다시 부르면 남은 청크부터 이어 갑니다. 번호 중복을 다시 검사하므로 재요청이 중복 삽입하지 않습니다.
