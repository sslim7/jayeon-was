...
| `cost` | ❌ | ✅ | 통화 한 건의 **실제 원가**(원). 단가 설정이 있고 사용량이 있을 때만. |
...
통화 한 건에 **실제로 얼마가 들었는지**를 서버가 원화로 계산해 `GET /calls/{id}` 응답에 싣는다. 앱은 단가를 모르고 그리기만 한다(§`internal/calls/cost.go`).

### 🔴 단가는 설정값이다 — 코드에 박지 않는다

이 프로젝트는 공급자 교체(Vertex AI·Bedrock 등)를 전제로 설계돼 있고, 공급자 단가는 우리 배포 주기와 무관하게 개정된다. 단가가 소스에 있으면 요금표가 바뀔 때마다 빌드→배포가 필요하고 **그 사이 화면의 금액은 조용히 틀린 채** 떠 있다. 환율도 같은 이유로 설정이다.

### 🔴 계산은 서버가 한다

앱에 단가를 내려보내고 앱이 곱하게 하면, 구버전 앱이 옛 단가로 계산한 금액을 계속 보여 준다 — 그 화면에는 틀렸다는 표시가 어디에도 없다. 서버가 원화 금액까지 계산해 내려보내면 단가가 바뀌어도 **앱 배포가 필요 없다.**

### 🔴 설정이 모자라면 계산하지 않는다

네 값(`CALL_ASR_USD_PER_HOUR`, `CALL_LLM_USD_PER_MILLION_INPUT_TOKENS`, `CALL_LLM_USD_PER_MILLION_OUTPUT_TOKENS`, `CALL_AI_USD_TO_KRW`)이 **전부** 있어야 계산한다. 하나라도 없으면 `cost` 필드가 아예 나가지 않고 앱은 비용 칸을 그리지 않는다.

- **기본값을 두지 않는 이유**: 이 숫자는 사용자가 **공급자 교체를 판단하는 근거**다. 그럴듯한 기본 단가를 박아 두면 설정 실수(오타, terraform apply 전 배포)가 「그럴듯하지만 틀린 금액」으로 나타나는데, 화면만 봐서는 진짜와 구분되지 않는다. 비용 칸이 없는 쪽이 낫다 — 없는 것은 눈에 띄지만 틀린 것은 눈에 띄지 않는다.
- **일부만 설정하면 안 되는 이유**: 앱은 「받아쓰기 170원」만 그리고 사용자는 그것을 한 건의 총액으로 읽는다. 분석 단가가 빠졌다는 사실은 화면 어디에도 나타나지 않는다.
- 단가 `0`(무료 구간)은 유효한 설정이다. 환경변수가 놓여 있었는지로 판단하고 값이 `0` 인지로 판단하지 않는다. **환율만 예외로 `0` 을 거부한다** — 환율이 0 이면 모든 금액이 0원이 되어 공짜로 보인다.

### 🔴 사용량이 없는 통화는 0원이 아니라 「없음」이다

사용량은 작업 문서의 `usage` 에 누적된다. 그것이 비어 있는 통화가 있다:

- 파이프라인 이전에 만들어진 통화
- 기기 업로드 경로(`PUT /calls/{id}`)로 저장돼 **작업 문서 자체가 없는** 통화
- 공급자를 한 번도 부르지 못하고 실패한 통화

이때 `cost` 를 **0원으로 내보내면 안 된다.** 0원은 "돈이 안 들었다"는 주장이고, 그 화면을 보고 단가를 판단하면 통째로 틀린다. 필드를 통째로 뺀다.

### 계산

| 단계 | 사용량 | 단가 |
| --- | --- | --- |
| 받아쓰기 | `usage.audioSeconds` (초) | `CALL_ASR_USD_PER_HOUR` ÷ 3600 |
| 분석(입력) | `usage.promptTokens` | `CALL_LLM_USD_PER_MILLION_INPUT_TOKENS` ÷ 1,000,000 |
| 분석(출력) | `usage.completionTokens` | `CALL_LLM_USD_PER_MILLION_OUTPUT_TOKENS` ÷ 1,000,000 |

USD 소계에 `CALL_AI_USD_TO_KRW` 를 곱해 원화로 만든다.

⚠️ **`reasoningTokens` 를 출력 토큰에 더하지 않는다.** OpenAI 호환 응답의 `completion_tokens` 에 추론 토큰이 **이미 포함**되어 있고 `reasoning_tokens` 는 그 안의 내역일 뿐이다(§`internal/callai/alibaba_llm.go`). 더하면 추론 토큰이 두 번 청구된 것처럼 보인다. 응답에는 그래도 싣는다 — 0 이 아니면 thinking 이 켜져 있다는 뜻이라 요금이 왜 높은지의 단서가 된다.

⚠️ **`audio_seconds` 는 통화 길이와 다를 수 있다.** 재시도한 전사의 초까지 누적된 값이다(실패한 시도의 요금도 실제로 청구되므로 성공분만 기록하면 청구서와 어긋난다).

### 응답에 사용량 원본을 함께 싣는 이유

🔴 금액만 있으면 나중에 「이 숫자가 왜 이렇지」를 확인할 길이 없다. 단가는 배포 환경에 있고 사용량은 작업 문서에 있어서, 응답에 둘 중 하나라도 빠지면 화면의 금액을 손으로 검산할 수 없다.

### 🔴 저장하지 않는다 / 목록에 넣지 않는다

- **저장 금지**: 단가는 개정되는데 저장된 금액은 계산 시점에 굳는다 — 단가를 고쳐도 옛 통화만 옛 금액을 보여 주고 그 차이를 설명할 방법이 화면에 없다. `listRecord` 가 저장 경로에서 이 값을 비우는 것이 그 장치다(`audio_url` 과 같은 자리).
- **목록 금지**: 금액의 근거인 사용량은 **작업 문서에만** 있다. 목록에 실으려면 한 페이지(최대 100건)마다 작업 문서를 그만큼 더 읽어야 한다 — 비용을 보여 주려고 조회 비용을 100배로 키우는 셈이다. 목록 화면(한 줄 아코디언)에 금액을 세울 자리도 없다.

### 표기 반올림은 앱이 한다

서버는 **0.0001원 단위까지** 내려보낸다. 🔴 정수 원으로 반올림해 보내면 짧은 통화의 1원 미만 비용이 0 이 되고, 앱은 「0원」밖에 그릴 수 없어 **공짜로 읽힌다.** 0.01원 단위로 미리 뭉개도 그보다 싼 공급자로 갈아탔을 때 모든 금액이 0 으로 붙어 비교 자체가 불가능해진다. 자르는 목적은 정밀도 축소가 아니라 부동소수 잔여물(`96.40000000000001`)을 응답에 싣지 않는 것뿐이다.

한국 원화 관례(`96원`)로 바꾸는 것은 화면의 일이다(§jayeon-app `src/lib/call-cost.ts`): 1원 이상은 정수로 반올림, 1원 미만은 소수점 두 자리, 그보다 작으면 「0.01원 미만」.
...
| `CALL_ASR_USD_PER_HOUR` | **비용을 계산하지 않는다** | 오디오 1시간당 ASR 단가(USD) |
| `CALL_LLM_USD_PER_MILLION_INPUT_TOKENS` | **비용을 계산하지 않는다** | LLM 입력 100만 토큰당 단가(USD) |
| `CALL_LLM_USD_PER_MILLION_OUTPUT_TOKENS` | **비용을 계산하지 않는다** | LLM 출력 100만 토큰당 단가(USD) |
| `CALL_AI_USD_TO_KRW` | **비용을 계산하지 않는다** | USD→KRW 환율. 🔴 `0` 은 설정 없음으로 본다 |
